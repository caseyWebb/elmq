...
- **`elmq get -f <FILE> <NAME>... [-f ...]`** — multi-file get in one invocation. Repeatable `-f`/`--file` flag groups a file path with one or more declaration names; multiple groups span files. Output framed as `## Module.decl` when `elm.json` is discoverable, `## file:decl` fallback otherwise. Collapses the `list` → N×`get` read-phase into a single turn. Backward-compatible with existing `get FILE NAME...` form. Captured in `openspec/changes/add-get-multi-file`.
- **`elmq grep --definitions --source`** — two composable flags on `grep`. `--definitions` filters to matches at the declaration name site (not call sites). `--source` replaces per-match locator lines with full declaration source blocks, deduped by `(file, decl)`. Together: `grep '^update$' --definitions --source` is a one-call definition lookup. Framed `## Module.decl` / `## file:decl` like `add-get-multi-file`. Captured in `openspec/changes/add-grep-source-flags`.
