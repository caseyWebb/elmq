...
- ✓ **`restructure-edit-surface` — landed.** Every write command moved to a uniform `<verb> <scope>` form (`set decl`, `set let`, `set case`, `rm decl`, `rm let`, `rm case`, `rm arg`, `rm variant`, `rm import`, `rename decl`, `rename let`, `rename arg`, `add arg`, `add variant`, `add import`); the bare-noun variants (`variant add`/`rm`, `import add`/`remove`) are gone. Flat verbs (`patch`, `expose`, `unexpose`, `mv`, `move-decl`) stay flat. `variant cases` remains the lone noun-first command because it's a read op. Three new writer submodules — `writer/let_binding.rs`, `writer/case_branch.rs`, `writer/function_arg.rs` — deliver AST-aware sub-declaration ops: upsert/remove/rename a let binding, upsert/remove a case branch, add/remove/rename a function parameter. New `src/analysis.rs` module hosts shared CST query helpers (`collect_let_sites`, `collect_case_sites_generic`, `collect_binders_in_scope`, `compute_site_keys`) used by both `variant.rs` and the sub-decl writers. Addressing is content-primary (`--name`/`--pattern`/`--on`) with `--line` as an ambiguity escalation using absolute file lines that match `get`'s output; every ambiguity error lists candidates inline so disambiguation is a single round-trip. New `--content` flag on `set decl` and `--body` flag on `set let`/`set case` are stdin alternatives for inline invocations. `--name` mismatch on `set decl` / `set let` is now an error instead of a silent rename-via-upsert footgun — agents use `rename decl`/`rename let` for actual renames. Every write emits `ok` on success; `rm variant`'s `references_not_rewritten` advisory is the documented exception. **BREAKING** across every write-side workflow — mechanical migration via the new command table in `src/guide.md`. 441 tests green. Captured in `openspec/changes/restructure-edit-surface`.
