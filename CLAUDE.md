...
- **`src/grep.rs`** — Grep command: `execute()` entry point with `GrepArgs` struct; scans Elm files for regex (or literal, via `-F`) matches and annotates each hit with its enclosing top-level declaration via a tree-sitter-built offset→decl map. Filters matches inside `--` / `{- -}` comments and string literals by default (opt back in with `--include-comments` / `--include-strings`). Project discovery walks up for `elm.json` honoring `source-directories`, or falls back to recursively walking the CWD when none is found; both paths honor `.gitignore` via the `ignore` crate. Emits compact `file:line:decl:text` or NDJSON. Exit codes match ripgrep (0/1/2).
